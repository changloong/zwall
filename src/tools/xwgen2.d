...
struct _G {
	static char[] to	= cast(char[]) "msd/" ;
	static char[] self	= cast(char[]) "" ;
	static char[] home	= cast(char[]) "" ;
	static char[][]
			ini_files	= [];
	...
	static void gen_ini(char[][] dirs){
		...
		foreach( dir; dirs  ){
			...
			foreach( _file ; fs.files ){
				auto p		= FNode.Open( cast(char[]) _file.toString, &tmp);
				if( p.isLink ){
...
struct BSDLite {
	...
	private static void _create( char[] ini_file ){
		char[] parent	= cast(char[]) "" ;
		foreach( _line ;  new Lines!(char)( new File( ini_file , File.ReadExisting) )  ){
			int i	= _A.find(_line, ';' ) ;
			char[] line = cast(char[]) Txt.trim(_line[ 0 .. i ] ) ;
			if( line.length < 1 ) 
				continue ;
			if( line.length > 2 ){
				if( line[0] == '['  && line[$-1] == ']' ){
					line	= cast(char[]) Txt.trim(line[1..$-1]);
		...
		foreach(int i, ref c; dir ){
			if( c == '/' ){
				...
				if( !_P.exists( to ) ) {
                    ...
                    int dir_mode    = 493 ; // 0755
    				if( from !is null ){
    					dir_mode    = from.mode ;
    				}
					mkdir( toStringz(to) , dir_mode );
                }
			}
		}
	}
	...
	static char[] relPath(char[] from, char[] to){
		...
		to_	= to_[_i..$];
		char[][] rel_;
		for( int i = 1; i < from_.length; i++) rel_ ~= cast(char[]) ".."; 
		for( int i = 0; i < to_.length; i++) rel_ ~= to_[i]; 
		rel	= Txt.join(rel_, cast(char[]) "/");
		return rel.dup ; 
	}
}

struct BSDLib{
	static const char[][]	dirs	= ["bin", "sbin", "usr/bin", "usr/sbin" , "usr/local/bin", "usr/local/sbin" ];
	...
	static void copy(){
		...
		foreach( dir ; dirs ){
			...
			fs(dir);
			foreach( _file ; fs.files ){
				FNode* f = FNode.Open(cast(char[]) _file.toString, &_fn );
				if( !f.isLink && f.isExe ){
					bu(" ")(f.Name);
				}
			}
		...
		auto p = new Process (cmd , null ) ;
                p.execute;
                bu.copy( p.stdout ) ;
                auto auto_libs = new File ("../xwall_libs.txt", File.WriteCreate);
                scope(exit) auto_libs.close ;
                char[][] libs    = [] ;
                scope ls        = new Lines!(char)(bu);
                foreach( line; ls ){
                        auto_libs.write(line);
                        auto_libs.write("\n") ;
